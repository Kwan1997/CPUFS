function [A, B, C, F, U, V] = tensorFS_guan_penaltyD(X, c, mu, alfa, beda, theda, verbose)
    ...
    while (iter < maxIter)
        ...
        while (cnt <= 2)
            ...
            % theda_U = linesearch_U(U, V, -dU, X, F, n, c, alfa, beda, theda);
            % U = U - theda_U .* dU;
            U = U - theda .* dU;
            ...
            % theda_V = linesearch_V(U, V, -dV, X, F, n, c, alfa, beda, theda);
            % V = V - theda_V .* dV;
            V = V - theda .* dV;
        ...
        if any(any(isnan(U))) || any(any(isinf(U))) || any(any(isnan(V))) || any(any(isinf(V)))
            ...
            [A, B, C, F, U, V] = tensorFS_guan_penaltyD(X, c, mu, alfa, beda, theda / 2, verbose);
            return;
        end
